...
std::ofstream out1("/Users/Carlos/Desktop/algohw1v3/test.txt");
...
public:
    Graph(int n, bool animation = true, double delay = 0) : n(n), animation(animation), delay(delay), recursiveCalls(-2), moves(0), movesValid(true){
        ...
        if (movesValid){
            std::cout << "All moves are valid" << endl;
        } else {
            std::cout << "Some moves are invalid (bigger disk on top of smaller disk)" << endl;
        }
    ...
    void moveNext(Node * a, Node * b) {
        if(b == d) {
            ...
            cout << a->v->getName() << " -> " << a->v->top() << " -> " << b->v->getName() << std::endl;
            b->v->push(a->v->pop());
            printGraph();
        } else {
            if(movesValid) movesValid = validMove(a,a->next);
